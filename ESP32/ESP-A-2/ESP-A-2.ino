...
#define PIN_COMPRESSOR 15
#define PIN_FAN_EVAP 21
#define ZC 19 //Zero-cross pin for dimmer
...
void tempandhumid(){  //Function to control temperature and humidity
    ...
    if(mode_status){  //if mode --> automatic
        if(temp_setpoint < temp_sensor_ambient){  //if temp setpoint below ambient temp
            ...
            digitalWrite(PIN_COMPRESSOR,LOW); //active low
            heater_status = 0;
            compressor_status = 1;
          }else if(temp_sensor < temp_setpoint - temp_hysterisis){
            dimmer_Heater.setState(OFF);
            digitalWrite(PIN_COMPRESSOR,HIGH); //active low
            heater_status = 0;
            compressor_status = 0;
          }else{  //when temperature reached, humidity control turn on
            if(humidity_sensor > humidity_setpoint + humidity_hyterisis){
              digitalWrite(PIN_HUMIDIFIER,HIGH); //active low
              digitalWrite(PIN_FAN_HUMIDIFIER,HIGH);
              humidifier_status = 0;
            } else if(humidity_sensor < humidity_setpoint - humidity_hyterisis){
              digitalWrite(PIN_HUMIDIFIER,LOW); //active low
              digitalWrite(PIN_FAN_HUMIDIFIER,LOW);
              humidifier_status = 1;
            }else {
    ...
    if(humidifier_status){
      digitalWrite(PIN_HUMIDIFIER,LOW); //active low
      digitalWrite(PIN_FAN_HUMIDIFIER,LOW); //active low
    }else {
      digitalWrite(PIN_HUMIDIFIER,HIGH);//active low
      digitalWrite(PIN_FAN_HUMIDIFIER,HIGH);//active low
    }
  }
