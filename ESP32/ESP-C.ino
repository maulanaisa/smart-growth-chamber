...
void readtemp_ambient() {
  temp_sensor_ambient = thermocouple.readCelsius();
   char sString[8];
   dtostrf(temp_sensor_ambient, 1, 0, sString);
   client.publish("esp32/tempambientsens", sString, true);
}

//Time to read ambient temperature sensor
unsigned long previousTimeTempAmbient = millis();
long timeIntervalTempAmbient = 300000;
...
void bHomeAutoPushCallback(void *ptr) {
  ...
  mode_status = 1;
  ActCallback();
  client.publish("esp32/mode", "1", true);
}
...
void bHomeManualPushCallback(void *ptr) {
  ...
  mode_status = 0;
  ActCallback();
  client.publish("esp32/mode", "0", true);
}
...
void Setcallback(void *ptr) {
  ...
  if (currentDisplay == HUM) {
    ...
    if ((fBuffer2 > 0.0000001) || (fBuffer2 = 0)) {
      ...
      client.publish("esp32/setpoint_humidity", sString, true);
    }
    ...
    if ((fBuffer2 > 0.0000001) || (fBuffer2 == 0)) {
      ...
      client.publish("esp32/setpoint_temperature", sString, true);
    }
  }
    ...
    if ((fBuffer2 > 0.0000001) || (fBuffer2 = 0)) {
      ...
      client.publish("esp32/setpoint_led", sString, true);
    }
  }
    ...
    if (humidifier_status) {
      client.publish("esp32/humidifier", "1", true);
    } else {
      client.publish("esp32/humidifier", "0", true);
    }
  }
    ...
    if (compressor_status) {
      client.publish("esp32/cooler", "1", true);
    } else {
      client.publish("esp32/cooler", "0", true);
    }
    ...
    if (heater_status) {
      client.publish("esp32/heater", "1", true);
    } else {
      client.publish("esp32/heater", "0", true);
    }
  }
    ...
    client.publish("esp32/fan", fanString, true);
  }
}
...
void ActCallback() {
  ...
  if (currentDisplay == HUM) {
    ...
    fBuffer2 = led_sensor;
    sprintf(buffer1, "%.0f", fBuffer2);
    tAuto_light.setText(buffer1);
    fBuffer2 = temp_sensor;
    sprintf(buffer2, "%.1f", fBuffer2);
